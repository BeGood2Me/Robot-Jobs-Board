...
Scan [C++ robotics jobs](/?tag=cpp) and you will see the same cluster:

1. **C++** for anything that runs on the robot.
2. **Python** for glue, notebooks, and tests.
3. **Linux** until it is boring.
4. **ROS 2** if the company is not a humanoid lab with a private stack. See [ROS 2 jobs](/?tag=ros2).
...
Simulation is the cheapest way to practice. Gazebo, Isaac Sim, and Webots all appear in [simulation tagged listings](/?tag=isaac-sim). A small stack that drives a differential robot, a manipulator, or a drone in sim is worth more than a certificate.

If you want warehouse work, add networking and the nerve to debug WiFi next to a charger. If you want aerial work, [PX4 jobs](/?tag=px4) and [drone jobs](/robots/drone-jobs) tell you whether clearance and on-site time are the real filter.
